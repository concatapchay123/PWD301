...
class StudentView {
  // =========================================================================
  // 0. Flyout Notification Hub (Right Slide-out Drawer)
  // =========================================================================
  static async openNotificationHub() {
    const activeRole = sessionStorage.getItem('active_role') || (window.app && window.app.currentUser && window.app.currentUser.primary_role) || 'STUDENT';
    const title = activeRole === 'ADMIN'
      ? 'Thông báo Học vụ & Hệ thống'
      : (activeRole === 'INSTRUCTOR' ? 'Thông báo Giảng dạy & Khảo thí' : 'Thông báo Học tập');

    UI.openDrawer({
      side: 'right',
      title: title,
      width: 'max-w-md',
      headerBadge: '<span id="hub-unread-badge" class="px-2 py-0.5 rounded-full bg-primary-subtle text-primary text-[10px] font-extrabold">Đang tải...</span>',
      bodyHtml: `
        <div class="space-y-4" id="notification-hub-root">
          <!-- Category Tabs -->
          <div class="flex items-center gap-1 border-b border-slate-200 dark:border-slate-800 text-xs font-bold pb-2" id="notif-category-tabs">
            <button type="button" class="notif-tab-btn px-3 py-1.5 rounded-lg bg-primary text-white transition-colors" data-category="ALL">Tất cả</button>
            <button type="button" class="notif-tab-btn px-3 py-1.5 rounded-lg text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors" data-category="ASSESSMENT">Khảo thí</button>
            <button type="button" class="notif-tab-btn px-3 py-1.5 rounded-lg text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors" data-category="COURSE">Khóa học</button>
            <button type="button" class="notif-tab-btn px-3 py-1.5 rounded-lg text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors" data-category="SYSTEM">Hệ thống</button>
          </div>

          <!-- Notification Items Container -->
          <div id="notif-items-list" class="space-y-2.5 min-h-[300px]">
            <div class="text-center py-16 text-slate-400 text-xs">
              <span class="inline-block animate-spin text-xl mb-2">⏳</span>
              <p>Đang nạp thông báo...</p>
            </div>
          </div>
        </div>
      `,
      footerHtml: `
        <div class="flex items-center justify-between w-full">
          <button type="button" id="notif-mark-all-read-btn" class="text-xs font-bold text-primary hover:underline flex items-center gap-1">
            <span class="material-symbols-outlined text-[16px]">done_all</span>
            <span>Đánh dấu tất cả đã đọc</span>
          </button>
          <button type="button" class="px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-bold hover:bg-slate-200 transition-colors" onclick="UI.closeDrawer()">
            Đóng
          </button>
        </div>
      `
    ...
    try {
      ...
      const badgeEl = document.getElementById('hub-unread-badge');
      if (badgeEl) {
        badgeEl.textContent = unreadCount > 0 ? `${unreadCount} mới` : '0 mới';
        if (unreadCount === 0) badgeEl.className = 'px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 text-[10px] font-bold';
      }
      ...
      const renderItems = () => {
        ...
        const filtered = items.filter(item => {
          ...
          const cat = (item.category || item.event_category || '').toUpperCase();
        ...
        if (filtered.length === 0) {
          listEl.innerHTML = `
            <div class="text-center py-12 text-slate-400 text-xs space-y-2">
              <span class="material-symbols-outlined text-3xl text-slate-300 dark:text-slate-700">notifications_off</span>
              <p class="font-medium">Không có thông báo nào trong danh mục này.</p>
            </div>
          `;
          return;
        }
        ...
        listEl.innerHTML = filtered.map(n => {
          ...
          const cat = (n.category || n.event_category || 'SYSTEM').toUpperCase();
          let icon = 'notifications';
          let iconColor = 'bg-primary-subtle text-primary';
          if (cat === 'ASSESSMENT') {
            icon = 'quiz';
          ...
          return `
            <div
              class="notif-item p-3.5 rounded-2xl border transition-all cursor-pointer flex items-start gap-3 ${isRead ? 'border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 opacity-75' : 'border-primary/30 bg-primary/[0.03] shadow-sm'}"
              data-id="${n.notification_id || n.id}"
              data-link="${UI.escapeHtml(n.target_url || n.link || '')}"
            >
              ...
              <div class="flex-1 min-w-0 space-y-1">
                <div class="flex items-center justify-between gap-2">
                  <span class="text-xs font-bold text-slate-900 dark:text-white truncate">${UI.escapeHtml(n.title || 'Thông báo mới')}</span>
                  ${!isRead ? '<span class="w-2 h-2 rounded-full bg-primary shrink-0"></span>' : ''}
                </div>
                <p class="text-[12px] text-slate-600 dark:text-slate-400 leading-snug line-clamp-2">${UI.escapeHtml(n.message || n.content || '')}</p>
                <div class="text-[10px] text-slate-400 pt-0.5">
                  ${UI.formatDateTime(n.created_at)}
                </div>
              </div>
            </div>
          `;
      ...
      document.querySelectorAll('.notif-tab-btn').forEach(btn => {
        btn.onclick = () => {
          ...
          document.querySelectorAll('.notif-tab-btn').forEach(b => {
            b.className = 'notif-tab-btn px-3 py-1.5 rounded-lg text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors';
          });
          btn.className = 'notif-tab-btn px-3 py-1.5 rounded-lg bg-primary text-white transition-colors';
          renderItems();
        };
      });
      ...
      if (markAllBtn) {
        markAllBtn.onclick = async () => {
          try {
            ...
            const b = document.getElementById('hub-unread-badge');
            if (b) {
              b.textContent = '0 mới';
              b.className = 'px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 text-[10px] font-bold';
            }
            window.Router?.refreshNotificationBadge?.();
            renderItems();
            UI.showToast('Đã đánh dấu tất cả thông báo là đã đọc.', 'success');
          } catch (e) {
            UI.showToast(e.message || 'Không thể đánh dấu đọc.', 'error');
          }
        };
      }
      ...
      console.warn('Failed to load notification hub:', err);
    }
  }

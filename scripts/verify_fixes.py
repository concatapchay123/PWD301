...
def run_checks():
    ...
    if r.headers.get("Content-Type", "").startswith("application/json"):
        ...
        assert "Đăng xuất" in r.text or "Giảng viên" in r.text or "Khóa học của tôi" in r.text, "Login failed"

    print("\n--- 3. Check Role Switching Menu in HTML ---")
    assert "KHÔNG GIAN LÀM VIỆC" not in r.text, "Role switching menu was found in base template! Should be removed."
    ...
    assert "bi-three-dots-vertical" in r.text or "Tùy chọn" in r.text, "3-dots menu not found in courses list!"
    ...
    for line in login_page.text.splitlines():
        if 'name="csrf_token"' in line:
            ...
            if m:
                csrf_token = m.group(1)
                break
    r = student_session.post(f"{BASE_URL}/auth/login", data={
    ...
    assert r.status_code == 200
    assert "var(--white)" in r.text or "var(--slate-50)" in r.text or "chat-interface-card" in r.text
